...
out = subprocess.Popen(['git', 'clone', 'git://github.com/dsoul/tv.styk.storage.oi.zfs.status.git', 'apps/status'], stdout=subprocess.PIPE).communicate()[0]
print out
print "To start zfs.status - source " + os.path.join(RUN_PATH, 'env') + '/zfs.status/bin/activate, cd apps/status, python status.pl, browse http://127.0.0.1/status'
